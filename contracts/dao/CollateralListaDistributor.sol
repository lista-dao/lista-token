...
contract CollateralListaDistributor is CommonListaDistributor {
    ...
    // token -> account -> debt
    mapping(address => uint256) public userInk;
    ...
    */
    function takeSnapshot(address _token, address _user, uint256 _ink) onlyRole(MANAGER) external {
        ...
        if (_ink > userInk[_user]) {
            _deposit(_user, _ink - userInk[_user]);
        } else {
            _withdraw(_user, userInk[_user] - _ink);
        }

        // update _ink amount of this collateral
        userInk[_user] = _ink;
    }
}
