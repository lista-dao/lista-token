    ...
    PausableUpgradeable
{
    ...
    mapping(address => mapping(uint16 => mapping(uint16 => mapping(address => bool))))
        public claimedIncentives;

    // distributorId -> week -> asset -> amount
    mapping(uint16 => mapping(uint16 => mapping(address => uint256)))
        public weeklyIncentives;
    ...
    function initialize(
        ...
        require(
            emissionVoting.hasRole(emissionVoting.ADMIN_VOTER(), _adminVoter),
            "Invalid adminVoter"
        );
    ...
    /**
     * @dev Whiltelist an asset for voting incentives
    ...
    function addIncentives(
        ...
        require(
            _distributorId > 0 && _distributorId <= vault.distributorId(),
            "Invalid distributorId"
        );
        require(
            _startWeek >= vault.getWeek(block.timestamp) &&
                _startWeek <= _endWeek,
        ...
        require(
            actualAmount > 0 && actualAmount <= _expectAmount,
            "Invalid amount"
        );
    ...
    function claim(
        ...
        require(msg.sender != adminVoter, "Invalid voter");
        require(
            _distributorId > 0 && _distributorId <= vault.distributorId(),
            "Invalid distributorId"
        );
        require(
            !claimedIncentives[msg.sender][_distributorId][_week][_asset],
            "Already claimed"
        );
        uint256 adminWeight = getRawWeight(adminVoter, _distributorId, _week);

        uint256 amountToClaim = calculateAmount(
            msg.sender,
            _distributorId,
            _week,
            _asset,
        ...
        claimedIncentives[msg.sender][_distributorId][_week][_asset] = true;
        IERC20(_asset).safeTransfer(msg.sender, amountToClaim);
    ...
    function calculateAmount(
        ...
        uint256 poolWeight = emissionVoting.getDistributorWeeklyTotalWeight(
            _distributorId,
            _week
        );
        ...
        uint256 factor = _adminWeight > 0
            ? (1e18 * poolWeight) / (poolWeight - _adminWeight)
            : 1e18;
    ...
    function setAdminVoter(
        ...
        require(
            _adminVoter != address(0) && _adminVoter != adminVoter,
            "Invalid adminVoter"
        );
        adminVoter = _adminVoter;
    ...
    function getRawWeight(
        ...
        uint16 _distributorId,
        uint16 _week
    ) public view returns (uint256 _weight) {
        uint256 index = emissionVoting.userVotedDistributorIndex(
            _account,
            _week,
            _distributorId
        );
        EmissionVoting.Vote[] memory votes = emissionVoting
            .getUserVotedDistributors(_account, _week);
        if (votes.length == 0) {
        ...
        for (uint16 id = 1; id <= maxDistributorId; id++) {
            for (uint16 week = startWeek; week <= endWeek; week++) {
                for (uint256 i = 0; i < _assets.length; i++) {
                    ...
                    uint256 adminWeight = getRawWeight(
                        adminVoter,
                        id,
                        week
                    );
    ...
    function getDistributorIncentives(
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            _incentives[i] = weeklyIncentives[_distributorId][_week][
                _assets[i]
            ];
        }
    }
