...
import { Upgrades } from "@openzeppelin/foundry-upgrades/src/LegacyUpgrades.sol";
...
contract BscUpgradeRevenueDistributor is Script {
  ...
  function setUp() public virtual {
    // ─── Step 1: Fetch Keys & Setup ───
    proxyAdminKey = vm.envUint("PRIVATE_KEY_PROXY_ADMIN");
    proxyAdminOwner = vm.addr(proxyAdminKey);
    console.log("ProxyAdmin owner acting as Deployer/Upgrader:", proxyAdminOwner);
  }

  function run() public {
    // ─── Step 2: Validate Storage Safety & Upgrade ───
    ...
    //  3. Call the ProxyAdmin to upgrade the proxy proxy instance.
    vm.startBroadcast(proxyAdminKey);
    
    Upgrades.upgradeProxy(
      listaRevenueDistributorProxy,
      "contracts/dao/ListaRevenueDistributor.sol:ListaRevenueDistributor",
      ""
    );
    
    vm.stopBroadcast();
    console.log("  Proxy upgrade executed and validated successfully.");
