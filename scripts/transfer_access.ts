...
const main = async () => {
  ...
  for (const address of proxyAddresses) {
    const Contract = await hre.ethers.getContractFactory("ERC20LpListaDistributor");
    const contract = await Contract.attach(address);
    const oldRoleAddress = "0x89e68b97466c65e215C0B13de256188867f358Ae";
    const adminRole = await contract.DEFAULT_ADMIN_ROLE();
    const adminAddress = "0x8d388136d578dCD791D081c6042284CED6d9B0c6";
    const mangerRole = await contract.MANAGER();
    const mangerAddress = "0x8d388136d578dCD791D081c6042284CED6d9B0c6";
    const pauserRole = await contract.PAUSER();
    const pauserAddress = "0xEEfebb1546d88EA0909435DF6f615084DD3c5Bd8";
    // grant pauser role
    await contract.grantRole(pauserRole, pauserAddress);
    // revoke old pauser role
    await contract.revokeRole(pauserRole, oldRoleAddress);
    // grant manager role
    await contract.grantRole(mangerRole, mangerAddress);
    // revoke old manager role
    await contract.revokeRole(mangerRole, oldRoleAddress);
    // grant admin role
