...
contract EmissionVotingTest is Test {
    ...
    function setUp() public {
        ...
        TransparentUpgradeableProxy emissionVotingProxy = new TransparentUpgradeableProxy(
            ...
            abi.encodeWithSignature("initialize(address,address,address,address,uint256)", multiSig, adminVoter, address(veLista), address(vault), 86400)
        );
    ...
    function test_user_voting() public {
        ...
        uint256 adminPeriod =
            veLista.startTime() + (veLista.getCurrentWeek() + 1) * emissionVoting.WEEK() - // veLista current timestamp
        ...
        assertEq(vault.getDistributorWeeklyEmissions(1, votingWeek), weeklyEmission * (10000 + 30000 + 500000) / totalWeight);
        assertEq(vault.getDistributorWeeklyEmissions(2, votingWeek), weeklyEmission * 23000 / totalWeight);
