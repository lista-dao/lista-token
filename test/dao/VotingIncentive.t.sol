...
contract VotingIncentiveTest is Test {
    ...
    function test_addIncentives() public {
        ...
        votingIncentive.addIncentives(1, 1, 2, makeAddr("asset2"), 100);
        ...
        assertEq(
            votingIncentive.weeklyIncentives(1, 1, address(asset1)),
            0.5 ether
        );
        assertEq(
            votingIncentive.weeklyIncentives(1, 2, address(asset1)),
            0.5 ether
        );
        assertEq(votingIncentive.weeklyIncentives(1, 3, address(asset1)), 0);
    }
    ...
    function test_claim_zero_admin_weight() public {
        ...
        vm.mockCall(
            address(emissionVoting),
            abi.encodeWithSignature(
                "getDistributorWeeklyTotalWeight(uint16,uint16)",
                uint16(1),
                uint16(1)
            ),
        ...
        user2Votes[0] = EmissionVoting.Vote({
            distributorId: 1,
            weight: 1
        });
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(
                EmissionVoting.getUserVotedDistributors.selector,
                address(user2),
                uint16(1)
            ),
    ...
    function test_claim_nonZero_admin_weight() public {
        ...
        adminVotes[0] = EmissionVoting.Vote({
            distributorId: 1,
            weight: 50
        });
        vm.mockCall(
